...
class TextProcessor:
    ...
    def process_text_query(self, query: str) -> TextProcessingResult:
        ...
        try:
            ...
            # Získání zdrojů z vector search (nejprve, aby fungovaly fallback data)
            sources = self._get_source_documents(processed_query)
            
            # Pokud máme zdroje, pokusíme se o RAG
            if sources:
                try:
                    ...
                    logger.warning(f"RAG chain selhala: {rag_error}, používám fallback odpověď")
                    result = self._create_fallback_answer(processed_query, sources)
            else:
                # Žádné zdroje - vytvoříme základní odpověď
                result = f"Omlouvám se, nenašel jsem relevantní informace o dotazu '{query}' v databázi SÚKL. Prosím zkuste být konkrétnější nebo se obraťte na lékárnu."
    ...
    def _create_fallback_answer(self, query: str, sources: List[Dict[str, Any]]) -> str:
        """
        Vytvoří fallback odpověď ze zdrojů když RAG nefunguje
        
        Args:
            query: Původní dotaz
            sources: Zdroje z vector search
            
        Returns:
            str: Fallback odpověď
        """
        if not sources:
            return f"Nenašel jsem informace o '{query}' v databázi."
            
        # Použijeme první zdroj pro vytvoření odpovědi
        best_source = sources[0]
        metadata = best_source
        
        # Vytvoření strukturované odpovědi z metadata
        answer_parts = []
        
        if metadata.get("name"):
            answer_parts.append(f"**Název přípravku**: {metadata['name']}")
            
        if metadata.get("active_ingredient"):
            answer_parts.append(f"**Účinná látka**: {metadata['active_ingredient']}")
            
        if metadata.get("strength") and metadata.get("form"):
            answer_parts.append(f"**Léková forma a síla**: {metadata['form']}, {metadata['strength']}")
            
        if metadata.get("indication"):
            answer_parts.append(f"**Indikace**: {metadata['indication']}")
            
        if metadata.get("dosage"):
            answer_parts.append(f"**Dávkování**: {metadata['dosage']}")
            
        if metadata.get("contraindication"):
            answer_parts.append(f"**Důležitá upozornění**: {metadata['contraindication']}")
            
        if metadata.get("prescription_required"):
            answer_parts.append(f"**Předpisovost**: {metadata['prescription_required']}")
            
        if answer_parts:
            return "\n".join(answer_parts) + "\n\n*Informace z testovacích dat - prosím ověřte v lékárně.*"
        else:
            return f"Našel jsem záznam pro '{query}', ale detailní informace nejsou dostupné."
