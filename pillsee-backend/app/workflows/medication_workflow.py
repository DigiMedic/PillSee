...
def add_safety_disclaimers(state: MedicationState) -> MedicationState:
    ...
    try:
        ...
        # Safe get medication info
        medication_info = state.get("medication_info") or {}
        
        # Kontrola kontraindikací
        if medication_info.get("contraindication"):
            safety_warnings.append("⚠️ Zkontrolujte kontraindikace před užitím")
            
        # Kontrola interakcí
        if medication_info.get("interactions"):  
            safety_warnings.append("⚠️ Informujte se o možných interakcích s jinými léky")
            
        # Kontrola předpisovosti
        prescription_req = medication_info.get("prescription_required", "")
        if prescription_req and "předpis" in prescription_req.lower():
            safety_warnings.append("⚠️ Tento lék vyžaduje lékařský předpis")
            
        # Obecná upozornění pro konkrétní typy léků
        med_name = medication_info.get("name", "")
        if med_name:
            med_name_lower = med_name.lower()
            if any(keyword in med_name_lower for keyword in ["antibio", "kortiko", "psycho"]):
...
def validate_and_enhance_info(state: MedicationState) -> MedicationState:
    ...
    try:
        # Safe get with default empty dict
        medication_info = state.get("medication_info") or {}
        ...
        # Ensure state has at least empty medication_info
        if not state.get("medication_info"):
            state["medication_info"] = {}
        return state
...
def create_medication_workflow() -> StateGraph:
    ...
    # Routing function
    def route_query(state: MedicationState) -> MedicationState:
        """Route na základě typu dotazu"""
        # LangGraph node functions must return the state dict
        return state
    
    # Přidání routing node
    workflow.add_node("route_query", route_query)
    # Conditional routing function
    def should_extract_image(state: MedicationState) -> str:
        """Decide the next step based on query type"""
        if state["query_type"] == "image":
