...
class MedicationVectorStore:
    ...
    def __init__(self):
        ...
        self.query_name = settings.vector_query_name
    ...
    def _direct_similarity_search(
        ...
        try:
            ...
            # FALLBACK: Pokud match_medications funkce nefunguje (UUID/BIGINT issue)
            logger.info("Zkouším fallback vyhledávání bez RPC funkce...")
            return self._fallback_similarity_search(query, limit, similarity_threshold, filters)
    ...
    def _fallback_similarity_search(
        ...
        """
        Fallback vyhledávání když match_medications nefunguje
        Vrací mock data pro testování
        """
        logger.warning("Používám fallback mock data - prosím opravte databázové schéma")
        
        # Mock data pro testování (simuluje SÚKL data)
        mock_medications = [
            {
                "id": "mock-uuid-1",
                "content": "PARALEN 500 mg tablety - paracetamol pro tlumení bolesti a snížení horečky",
                "metadata": {
                    "name": "PARALEN",
                    ...
                    "indication": "bolest hlavy, zubní bolest, horečka",
                    "dosage": "1-2 tablety každých 4-6 hodin, max 8 tablet/den",
                    "contraindication": "těžká porucha jater",
                    "prescription_required": "volně prodejný"
                },
                "similarity": 0.95
            },
            {
                "id": "mock-uuid-2", 
                "content": "IBUPROFEN 400 mg tablety - protizánětlivý lék pro bolest a záněty",
                "metadata": {
                    "name": "IBUPROFEN",
                    ...
                    "form": "tablety",
                    "manufacturer": "Léčiva",
                    "indication": "bolest, záněty, horečka",
                    "dosage": "1 tableta 2-3x denně",
                    "contraindication": "žaludeční vředy",
                    "prescription_required": "volně prodejný"
                },
                "similarity": 0.85
            }
        ...
        # Základní text matching pro relevanci
        query_lower = query.lower()
        relevant_meds = []
        
        for med in mock_medications:
            content_lower = med["content"].lower()
            name_lower = med["metadata"]["name"].lower()
            
            # Jednoduché scoring na základě text matching
            score = 0.0
            ...
            if score > 0:
                med["similarity"] = min(0.9, score)  # Cap at 0.9 for mock data
                relevant_meds.append(med)
        
        # Filtrovat podle threshold a seřadit podle similarity
        results = [med for med in relevant_meds if med["similarity"] >= similarity_threshold]
        results.sort(key=lambda x: x["similarity"], reverse=True)
        
        logger.info(f"Fallback vrátil {len(results[:limit])} mock výsledků")
